...
Trading Core System은 바이낸스 API를 활용하여 실시간 데이터 수집, 분석, 자동 매매를 수행하는 백엔드 시스템입니다.
...
│   ├── main/
│   │   ├── kotlin/
│   │   │   └── com/xijjun/trading/tradingcore/
│   │   │       └── TradingCoreApplication.kt
│   │   └── resources/
│   │       ├── application.properties
│   │       ├── application-local.properties
│   │       ├── application-sandbox.properties
│   │       └── db/
│   │           └── migrations/          # Flyway 마이그레이션 파일
│   └── test/
│       └── kotlin/
│           └── com/xijjun/trading/tradingcore/
│               └── TradingCoreApplicationTests.kt
├── docs/                                # 📚 문서 디렉토리 (아래 참고)
...
- **[MODULITH_STRATEGY.md](docs/architecture/MODULITH_STRATEGY.md)**: Spring Modulith 전략
  - 모듈 구조 및 패키지 설계
  - 모듈 간 통신 방식
  - 마이크로서비스 전환 전략

#### 개발 가이드
- **[DB_MIGRATION_GUIDE.md](docs/develop-guide/DB_MIGRATION_GUIDE.md)**: DB 마이그레이션 가이드
  - Flyway 사용 방법
  - 마이그레이션 파일 작성 규칙
...
### 모듈 구성
1. **데이터 수집 모듈**: 바이낸스 API를 통한 캔들스틱 데이터 수집
2. **데이터 가공 모듈**: 기술적 지표 계산 (SMA, RSI)
3. **코인 매매 모듈**: 자동 주문 실행 및 포지션 관리
4. **리스크 관리 모듈**: 독립 모듈로 분리된 리스크 검증
5. **AI 모듈**: 규칙 기반 판단 (MVP), AI 모델 통합 (Phase 2)
6. **API Logic 모듈**: REST API 제공
...
### MVP 범위
- 1개 코인 (BTC/USDT) 5분봉 데이터 수집
- 기본 기술적 지표 계산 (SMA, RSI)
- 규칙 기반 매매 판단 (RSI 기반)
- Long 포지션 자동 매매 (시장가)
- 리스크 관리 모듈
- 기본 REST API

자세한 내용은 [MVP_SCOPE.md](docs/MVP_SCOPE.md)를 참고하세요.
...
태스크 목록은 [MVP_TASKS.md](docs/MVP_TASKS.md)를 참고하세요.
...
# 특정 테스트 실행
./gradlew test --tests "com.xijjun.trading.tradingcore.*"
```
